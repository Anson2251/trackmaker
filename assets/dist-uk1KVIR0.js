import { C as e, M as t, N as n, m as r, n as i, r as a, s as o, t as s, u as c, v as ee, y as l } from "./dist-CQp5Y4zm.js";
import { i as u } from "./dist-CjAScDBv.js";
var d = 168, f = 169, p = 170, m = 1, te = 2, h = 3, ne = 171, g = 172, _ = 4, v = 173, y = 5, b = 174, x = 175, S = 176, C = 177, w = 6, T = 7, E = 8, re = 9, D = 0, O = [9, 10, 11, 12, 13, 32, 133, 160, 5760, 8192, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200, 8201, 8202, 8232, 8233, 8239, 8287, 12288], ie = 58, k = 40, A = 95, j = 91, M = 45, N = 46, P = 35, F = 37, I = 123, L = 125, R = 47, z = 42, B = 10, V = 61, H = 43, U = 38;
function W(e2) {
  return e2 >= 65 && e2 <= 90 || e2 >= 97 && e2 <= 122 || e2 >= 161;
}
function G(e2) {
  return e2 >= 48 && e2 <= 57;
}
function K(e2) {
  let t2;
  return e2.next == R && ((t2 = e2.peek(1)) == R || t2 == z);
}
var q = new i((e2, t2) => {
  if (t2.dialectEnabled(D)) {
    let n2;
    if (e2.next < 0 && t2.canShift(S)) e2.acceptToken(S);
    else if (((n2 = e2.peek(-1)) == B || n2 < 0) && t2.canShift(x)) {
      let t3 = 0;
      for (; e2.next != B && O.includes(e2.next); ) e2.advance(), t3++;
      e2.next == B || K(e2) ? e2.acceptToken(x, -t3) : t3 && e2.acceptToken(C);
    } else if (e2.next == B) e2.acceptToken(b, 1);
    else if (O.includes(e2.next)) {
      for (e2.advance(); e2.next != B && O.includes(e2.next); ) e2.advance();
      e2.acceptToken(C);
    }
  } else {
    let t3 = 0;
    for (; O.includes(e2.next); ) e2.advance(), t3++;
    t3 && e2.acceptToken(C);
  }
}, { contextual: true }), J = new i((e2, t2) => {
  if (K(e2)) {
    if (e2.advance(), t2.dialectEnabled(D)) {
      let t3 = -1;
      for (let n2 = 1; ; n2++) {
        let r2 = e2.peek(-n2 - 1);
        if (r2 == B || r2 < 0) {
          t3 = n2 + 1;
          break;
        } else if (!O.includes(r2)) break;
      }
      if (t3 > -1) {
        let n2 = e2.next == z, r2 = 0;
        for (e2.advance(); e2.next >= 0; ) if (e2.next == B) {
          e2.advance();
          let n3 = 0;
          for (; e2.next != B && O.includes(e2.next); ) n3++, e2.advance();
          if (n3 < t3) {
            r2 = -n3 - 1;
            break;
          }
        } else if (n2 && e2.next == z && e2.peek(1) == R) {
          r2 = 2;
          break;
        } else e2.advance();
        e2.acceptToken(n2 ? T : w, r2);
        return;
      }
    }
    if (e2.next == R) {
      for (; e2.next != B && e2.next >= 0; ) e2.advance();
      e2.acceptToken(w);
    } else {
      for (e2.advance(); e2.next >= 0; ) {
        let { next: t3 } = e2;
        if (e2.advance(), t3 == z && e2.next == R) {
          e2.advance();
          break;
        }
      }
  ...
  (e2.next == H || e2.next == V) && t2.dialectEnabled(D) && e2.acceptToken(e2.next == V ? E : re, 1);
}), ae = new i((e2, t2) => {
  if (!t2.dialectEnabled(D)) return;
  let n2 = t2.context.depth;
  if (e2.next < 0 && n2) {
    e2.acceptToken(f);
    return;
  }
