import { A as e, C as t, I as n, M as r, N as i, et as a, g as o, n as s, r as c, s as l, u, wt as d, y as f } from "./dist-CQp5Y4zm.js";
import { n as p } from "./dist-DYp_OIFV.js";
import "./dist-CjAScDBv.js";
import "./dist-Doi9LDAp.js";
var m = 1, h = 2, g = 3, _ = 180, v = 4, y = 181, b = 5, x = 182, S = 6;
function C(e2) {
      ...
      if (n3 == 123) {
        ...
        for (; ; ) {
          ...
          if (t3 == 32 || t3 == 10) ++n4;
            ...
            let i2 = t3 == 101 && e2.peek(n4 + 1) == 110 && e2.peek(n4 + 2) == 100;
            e2.acceptToken(i2 ? g : h, r2);
            return;
          }
        }
      }
...
function R(t2) {
  ...
  if (t2.explicit && a2 == `|`) return { type: `filter` };
  if (i2.name == `TagName`) return { type: `tag`, node: i2 };
  if (t2.explicit && a2 == `{%`) return { type: `tag` };
  if (i2.name == `PropertyName` && i2.parent.name == `MemberExpression`) return { type: `property`, node: i2, target: i2.parent };
  if (i2.name == `.` && i2.parent.name == `MemberExpression`) return { type: `property`, target: i2.parent };
  if (i2.name == `MemberExpression` && a2 == `.`) return { type: `property`, target: i2 };
  if (i2.name == `VariableName`) return { type: `expression`, from: i2.from };
  let o2 = t2.matchBefore(/[\w\u00c0-\uffff]+$/);
  return o2 ? { type: `expression`, from: o2.from } : t2.explicit && i2.name != `CommentText` && i2.name != `StringLiteral` && i2.name != `NumberLiteral` && i2.name != `InlineComment` ? { type: `expression` } : null;
}
function z(e2, t2, n2, r2) {
  ...
  for (; ; ) {
    ...
    if (!n3) return [];
    if (n3.name == `forloop`) return i2.length ? [] : I;
    if (n3.name == `tablerowloop`) return i2.length ? [] : L;
  ...
  return r2 ? r2(i2, e2, n2) : [];
}
function B(e2 = {}) {
  let t2 = e2.filters ? e2.filters.concat(N) : N, n2 = e2.tags ? e2.tags.concat(P) : P, r2 = e2.variables ? e2.variables.concat(F) : F, { properties: i2 } = e2;
  return (e3) => {
    let a2 = R(e3);
    if (!a2) return null;
    let o2 = a2.from ?? (a2.node ? a2.node.from : e3.pos), s2;
    return s2 = a2.type == `filter` ? t2 : a2.type == `tag` ? n2 : a2.type == `expression` ? r2 : z(e3.state, a2.target, e3, i2), s2.length ? { options: s2, from: o2, validFor: /^[\w\u00c0-\uffff]*$/ } : null;
  };
}
var V = n.inputHandler.of((e2, t2, n2, r2) => r2 != `%` || t2 != n2 || e2.state.doc.sliceString(t2 - 1, n2 + 1) != `{}` ? false : (e2.dispatch(e2.state.changeByRange((e3) => ({ changes: { from: e3.from, to: e3.to, insert: `%%` }, range: a.cursor(e3.from + 1) })), { scrollIntoView: true, userEvent: `input.type` }), true));
function H(e2) {
  return (t2) => {
...
var U = l.define({ name: `liquid`, parser: j.configure({ props: [r({ "cycle comment endcomment raw endraw echo increment decrement liquid in with as": i.keyword, "empty forloop tablerowloop": i.atom, "if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue": i.controlKeyword, "assign capture endcapture": i.definitionKeyword, contains: i.operatorKeyword, "render include": i.moduleKeyword, VariableName: i.variableName, TagName: i.tagName, FilterName: i.function(i.variableName), PropertyName: i.propertyName, CompareOp: i.compareOperator, AssignOp: i.definitionOperator, LogicOp: i.logicOperator, NumberLiteral: i.number, StringLiteral: i.string, BooleanLiteral: i.bool, InlineComment: i.lineComment, CommentText: i.blockComment, "{% %} {{ }}": i.brace, "[ ]": i.bracket, "( )": i.paren, ".": i.derefOperator, ", .. : |": i.punctuation }), t.add({ Tag: o({ closing: `%}` }), "UnlessDirective ForDirective TablerowDirective CaptureDirective": H(/^\s*(\{%-?\s*)?end\w/), IfDirective: H(/^\s*(\{%-?\s*)?(endif|else|elsif)\b/), CaseDirective: H(/^\s*(\{%-?\s*)?(endcase|when)\b/) }), f.add({ "UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment"(e2) {
  let t2 = e2.firstChild, n2 = e2.lastChild;
  return !t2 || t2.name != `Tag` ? null : { from: t2.to, to: n2.name == `EndTag` ? n2.from : e2.to };
} })] }), languageData: { commentTokens: { line: `#` }, indentOnInput: /^\s*{%-?\s*(?:end|elsif|else|when|)$/ } }), W = p();
function G(e2) {
  return U.configure({ wrap: d((t2) => t2.type.isTop ? { parser: e2.parser, overlay: (e3) => e3.name == `Text` || e3.name == `RawText` } : null) }, `liquid`);
}
var K = G(W.language);
function q(e2 = {}) {
  let t2 = e2.base || W, n2 = t2.language == W.language ? K : G(t2.language);
  return new u(n2, [t2.support, n2.data.of({ autocomplete: B(e2) }), t2.language.data.of({ closeBrackets: { brackets: [`{`] } }), V]);
}
export {
  V as closePercentBrace,
  q as liquid,
  B as liquidCompletionSource,
  K as liquidLanguage
};
