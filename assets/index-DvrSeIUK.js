import { n as u, o as x, p, b as h, L as y, q as X, g as b, a as Z, s as j, i as G, f as W, t as n, j as V, E as c } from "./index-CbBO3481.js";
...
function M(O) {
  ...
  return Q ? { type: "expression", from: Q.from } : O.explicit && $.name != "CommentText" && $.name != "StringLiteral" && $.name != "NumberLiteral" && $.name != "InlineComment" ? { type: "expression" } : null;
}
function J(O, a, e, r) {
  let $ = [];
  for (; ; ) {
...
function K(O = {}) {
  let a = O.filters ? O.filters.concat(q) : q, e = O.tags ? O.tags.concat(m) : m, r = O.variables ? O.variables.concat(T) : T, { properties: $ } = O;
  return (i) => {
    var Q;
    let t = M(i);
    if (!t) return null;
    let o = (Q = t.from) !== null && Q !== void 0 ? Q : t.node ? t.node.from : i.pos, s;
    return t.type == "filter" ? s = a : t.type == "tag" ? s = e : t.type == "expression" ? s = r : s = J(i.state, t.target, i, $), s.length ? { options: s, from: o, validFor: /^[\w\u00c0-\uffff]*$/ } : null;
  };
}
const OO = u.inputHandler.of((O, a, e, r) => r != "%" || a != e || O.state.doc.sliceString(a - 1, e + 1) != "{}" ? false : (O.dispatch(O.state.changeByRange(($) => ({ changes: { from: $.from, to: $.to, insert: "%%" }, range: x.cursor($.from + 1) })), { scrollIntoView: true, userEvent: "input.type" }), true));
function d(O) {
  return (a) => {
...
const $O = y.define({ name: "liquid", parser: A.configure({ props: [j({ "cycle comment endcomment raw endraw echo increment decrement liquid in with as": n.keyword, "empty forloop tablerowloop": n.atom, "if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue": n.controlKeyword, "assign capture endcapture": n.definitionKeyword, contains: n.operatorKeyword, "render include": n.moduleKeyword, VariableName: n.variableName, TagName: n.tagName, FilterName: n.function(n.variableName), PropertyName: n.propertyName, CompareOp: n.compareOperator, AssignOp: n.definitionOperator, LogicOp: n.logicOperator, NumberLiteral: n.number, StringLiteral: n.string, BooleanLiteral: n.bool, InlineComment: n.lineComment, CommentText: n.blockComment, "{% %} {{ }}": n.brace, "[ ]": n.bracket, "( )": n.paren, ".": n.derefOperator, ", .. : |": n.punctuation }), G.add({ Tag: V({ closing: "%}" }), "UnlessDirective ForDirective TablerowDirective CaptureDirective": d(/^\s*(\{%-?\s*)?end\w/), IfDirective: d(/^\s*(\{%-?\s*)?(endif|else|elsif)\b/), CaseDirective: d(/^\s*(\{%-?\s*)?(endcase|when)\b/) }), W.add({ "UnlessDirective ForDirective TablerowDirective CaptureDirective IfDirective CaseDirective RawDirective Comment"(O) {
  let a = O.firstChild, e = O.lastChild;
  return !a || a.name != "Tag" ? null : { from: a.to, to: e.name == "EndTag" ? e.from : O.to };
} })] }), languageData: { commentTokens: { line: "#" }, indentOnInput: /^\s*{%-?\s*(?:end|elsif|else|when|)$/ } }), P = p();
function f(O) {
  return $O.configure({ wrap: X((a) => a.type.isTop ? { parser: O.parser, overlay: (e) => e.name == "Text" || e.name == "RawText" } : null) }, "liquid");
}
const aO = f(P.language);
function QO(O = {}) {
  let a = O.base || P, e = a.language == P.language ? aO : f(a.language);
  return new h(e, [a.support, e.data.of({ autocomplete: K(O) }), a.language.data.of({ closeBrackets: { brackets: ["{"] } }), OO]);
}
export {
