...
export const versionIdentifierStorage = localforage.createInstance({ name: "trackmaker_db_version-identifiers" });

export async function initStorage(space: string, versionIdentifier: string, currentVersion: string) {
    const currentVersionDB = await versionIdentifierStorage.getItem<string>(versionIdentifier);
