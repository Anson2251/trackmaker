...
const getMostRecentCommitId = async () => new Promise<string>((resolve, reject) => {
...
async function generateConfig(): Promise<UserConfig> {
    const releaseMode = Boolean(JSON.parse((process.env.RELEASE_MODE ?? "false").toLowerCase()));
    const tauriEnv = Boolean(JSON.parse((process.env.TAURI_ENVIRONMENT ?? "false").toLowerCase()));
